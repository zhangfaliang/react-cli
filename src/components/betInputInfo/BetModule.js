...
class BetModule extends Component {
  ...
  handleKeyboardChange = value => {
    ...
    keyboardValue =
      dotIndex > -1 ? keyboardValue.slice(0, dotIndex) : keyboardValue
    this.setState({
      keyboardValue,
  ...
  handleAllin = activeFlag => {
    ...
    const keyboardValue =
      dotIndex > -1 ? String(allAmount).slice(0, dotIndex) : allAmount
    this.setState({
      keyboardValue,
      possibleWin: formatMoney(sp * Number(keyboardValue)),
      allInDisabled: true,
  ...
  render() {
    ...
    return (
      <div
        className={clsName}
        onClick={maskClosable && this.handleCloseKeyboard}
      >
        {childrens}
      </div>
    )
